...
// Internal test codes, for taking a real payment end to end without putting a
// $1 price on a public product page.
//
// Gated, unlike the first time round. These were previously in the list above
// and worked on production for anyone who typed them — a 99% discount sitting
// behind a guessable code on a live store. Now they resolve only when
// ALLOW_TEST_COUPONS is set, so enabling them is a deliberate act on the
// environment that needs them, and production stays clean by default.
//
// Percent, never fixed: a fixed discount larger than the order floors the total
// at $0, and a $0 charge is exactly the state that marks an order paid with no
// money collected. MIN_CHARGE below exists for the same reason — APEXTEST100 is
// 99.99% rather than 100% so there is always something real to charge.
const TEST_COUPONS = [
...
export const testCouponsEnabled = process.env.ALLOW_TEST_COUPONS === '1';

export function findCoupon(code) {
  if (!code) return null;
  const wanted = String(code).trim().toLowerCase();
  const pool = testCouponsEnabled ? [...COUPONS, ...TEST_COUPONS] : COUPONS;
  return pool.find((c) => c.code.toLowerCase() === wanted) || null;
}
